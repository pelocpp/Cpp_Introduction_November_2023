...
namespace Rectangle {
    ...
    void test03_center()
    {
        Rectangle rect1(1, 3, 3, 1);  // rect1: create & constructor

        rect1.print();

        Point c = rect1.center();     // c:  Wird durch Rückgabe von center bereits gestellt
                                      // being provided by being returned from invocation of method center

        std::cout << "Center: (" << c.getX() << "," << c.getY() << ')' << std::endl;
